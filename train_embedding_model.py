...
if __name__ == '__main__':
    ...
    use_sub_trainset_to_validateset = cfg_dict.get("use_sub_trainset_to_validateset")
    ...
    dataset_info, classes = load_dataset(train_dataset_dir, ratio=[train_ratio, val_ratio, test_ratio])
    ...
    data_loader = load_dataloader(**dataset_info, batchsize=batchsize, num_workers=num_workers,
                                  train_transforms=train_transforms, val_transforms=val_transforms,
                                  use_letterbox=use_letterbox, sample_count=-1, m_sample_per_class=m_sample_per_class)
    ...
    val_dataset = data_loader["val_set"]
    test_dataset = data_loader["test_set"]

    train_dataset_metric = train_dataset
    val_dataset_metric = val_dataset
    if use_sub_trainset_to_validateset:
        train_dataset_metric = ImageDataset(dataset_info["train_inputs"], dataset_info["train_labels"], val_transforms,
                                            use_letterbox, sample_count=200000)
        val_dataset_metric = ImageDataset(dataset_info["val_inputs"], dataset_info["val_labels"], val_transforms,
                                          use_letterbox, sample_count=-1)
