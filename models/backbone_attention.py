...
from models.STN import STNFC, STNFullyConv
from models.resnet_attention import ResNetAttention


class Backbone(object):
    def __init__(self, in_channels=3, out_dimension=10, model_name="resnet50", pretrained=False, model_path=None):
    ...
    def build_model(self):
        ...
        if self.model_name.startswith("attention"):
            self._build_attention_resnet()
    ...
    def _build_resnet(self):
        ...
        #     self.pretrained_parameters = []
        # else:
        #     self.train_parameters_id = list(map(id, self.model[1].fc.parameters())) + list(
        #         map(id, self.model[0].parameters()))
        #     print(self.train_parameters_id)
        #     self.pretrained_parameters = filter(lambda p: id(p) not in self.train_parameters_id,
        #                                         self.model.parameters())
        #     self.train_parameters = filter(lambda p: id(p) in self.train_parameters_id,
        #                                    self.model.parameters())
    ...
    def _build_attention_resnet(self):
        attention_name = self.model_name.split("_")[-1]
        model_name = self.model_name.split("_")[1]
        print(attention_name, model_name)
        self.model = ResNetAttention(model_name, classes=self.out_dimension, use_attention=True, attention_name=attention_name)

        if not self.pretrained:
            self.train_parameters = self.model.parameters()
            self.pretrained_parameters = []
        else:
            state_dict = self.model.load_state(self.model_path)

            self.train_parameters_id = []
            self.pretrained_parameters_id = []
            for name, param in self.model.named_parameters(recurse=True):
                if name in state_dict.keys():
                    self.pretrained_parameters_id.append(id(param))
                else:
                    self.train_parameters_id.append(id(param))
            print(self.train_parameters_id)
            print(self.pretrained_parameters_id)
            self.pretrained_parameters = filter(lambda p: id(p) in self.pretrained_parameters_id, self.model.parameters())
            self.train_parameters = filter(lambda p: id(p) in self.train_parameters_id, self.model.parameters())
...
if __name__ == '__main__':
    model_names = [
        ...
        "attention_resnet18_GCNet"
    ]
    device = torch.device("cpu")
    data = torch.randn((2, 3, 128, 128), device=device)
    for model_name in model_names:
        print(model_name)
        backbone = Backbone(out_dimension=10, model_name=model_name, pretrained=True,
                            model_path="../pretrained_models/hub/checkpoints/resnet18-f37072fd.pth")
        model, train_params, pretrained_params = backbone.build_model()
        print(list(map(lambda x: id(x), train_params)))
        print(list(map(lambda x: id(x), pretrained_params)))
        print(list(map(lambda x: id(x), model.parameters())))
        res = model(data)
        print(res.shape)
